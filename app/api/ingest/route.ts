...
function isPrivateHostname(hostname: string) {
  return hostname === "localhost" || hostname === "127.0.0.1" || hostname.startsWith("10.") || hostname.startsWith("192.168.") || hostname.startsWith("172.16.") || hostname.startsWith("172.17.") || hostname.startsWith("172.18.") || hostname.startsWith("172.19.") || hostname.startsWith("172.2");
}
...
export async function POST(req: Request) {
  try {
    ...
    if (isPrivateHostname(parsed.hostname)) throw new Error("ローカルネットワーク宛のURLは取得できません。");
    ...
    if (buf.byteLength > MAX_BYTES) throw new Error("画像サイズが大きすぎます。");
